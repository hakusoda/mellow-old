...
export function getRobloxLink(linkId: string) {
	return supabase.from('roblox_links')
		.select('*')
		.eq('id', linkId);
}
...
export async function getUsersByDiscordId(userIds: string[]) {
	const { data, error } = await supabase.from('user_connections')
		.select('sub, user:users ( id, bio, name, flags, username, created_at, avatar_url, primary_roblox_link_id, roblox_oauth_tokens:mellow_roblox_oauth_tokens ( expires_in, created_at, access_token, refresh_token ) )')
		.in('sub', userIds);
	if (error)
		console.error(error);
